...
int	main (int argc, char *argv[])
{
	...
	{
	Profiler &prof = getProfiler(PROF_GRAVI);
	...
	for (int it=0; it<nSteps ; it++)
	{
		...
		/* reserve ENERGY to m2half (ghosted and padded)*/
		prof.start();
		memmove(axion->m2half(),axion->m2Cpu(),axion->eSize()*axion->Precision());
		prof.stop();
		prof.add(std::string("hybrid-move"), 0.0, 0.0);
		...
		prof.stop();
		prof.add(std::string("hybrid-SOR"), 0.0, 0.0);

		prof.start();
		/* build reduced grid*/
		rfuap(axion, redfft);
		prof.stop();
		prof.add(std::string("hybrid-RFUAP"), 0.0, 0.0);
		...
		/* divide by k*2 */
		prof.stop();
		prof.add(std::string("hybrid-FFT"), 0.0, 0.0);

		prof.start();
		ilap(axion,redfft,smsteps);
		prof.stop();
		prof.add(std::string("hybrid-LAP"), 0.0, 0.0);
...
prof.start();
		...
		eguf(axion,redfft);
prof.stop();
prof.add(std::string("hybrid-EGUF"), 0.0, 0.0);
...
prof.add(std::string("hybrid-SOR"), 0.0, 0.0);
	...
	char *MS  = static_cast<char *> ((void *) axion->m2Start());
	size_t dl = axion->Precision()*Lx;
	size_t pl = axion->Precision()*(Lx+2);
	size_t ss = Lz*Lx;
	myPlan = AxionFFT::fetchPlan("pSpecAx");


{
	Profiler &prof = getProfiler(PROF_GRAVI2);
...
prof.start();
		munge(UNFOLD_M2);
prof.stop();
prof.add(std::string("full-fold"), 0.0, 0.0);
...
prof.stop();
prof.add(std::string("full-move"), 0.0, 0.0);

prof.start();
		/* fft */
		myPlan.run(FFT_FWD);
prof.stop();
prof.add(std::string("full-FFT"), 0.0, 0.0);
...
prof.stop();
prof.add(std::string("full-LAP"), 0.0, 0.0);
...
prof.add(std::string("full-FFT"), 0.0, 0.0);
		...
		/* We would need to change something to unfold to m2S from m2 */
