...
# ID
ups = 'N'+str(sizeN)+' L'+str(sizeL)+' n'+str(nqcd)
#+' ('+mark+')'+str(mac)
print('ID = '+ups)
print()
...
# THETA EVOLUTION
plt.clf()   
plt.plot(ztab1,Thtab1,linewidth=0.7)
plt.plot(ztab2,Thtab2,linewidth=0.7)
plt.ylim([-3.15,3.15])
plt.ylabel("theta")
plt.xlabel('tau')
plt.title(ups)
...
plt.clf()
plt.semilogy(en[:,0],sca*(en[:,1:4].sum(axis=1))/16.82,   c='b',label='Gtheta',linewidth=1,marker='.',markersize=0.1)
plt.semilogy(en[:,0],sca*(en[:,4])/16.82,                 c='k',label='Vtheta',linewidth=1,marker='.',markersize=0.1)
plt.semilogy(en[:,0],sca*(en[:,5])/16.82,                 c='r',label='Ktheta',linewidth=1,marker='.',markersize=0.1)
plt.semilogy(en[:,0],sca*(en[:,1:6].sum(axis=1))/16.82,   c='k',label='theta',linewidth=1,marker='.',markersize=0.1)
plt.semilogy(en[:ik,0],sca[:ik]*(en[:ik,6:9].sum(axis=1))/16.82,   '--',c='b',label='Grho',linewidth=0.5,marker='.',markersize=0.1)
plt.semilogy(en[:ik,0],sca[:ik]*(en[:ik,9])/16.82,                 '--',c='k',label='Vrho',linewidth=0.5,marker='.',markersize=0.1)
plt.semilogy(en[:ik,0],sca[:ik]*(en[:ik,10])/16.82,                '--',c='r',label='Krho',linewidth=0.5,marker='.',markersize=0.1)
plt.semilogy(en[:ik,0],sca[:ik]*(en[:ik,6:11].sum(axis=1))/16.82,  '--',c='k',label='rho',linewidth=0.5,marker='.',markersize=0.1)
plt.ylim([0.01,1000])
plt.grid(axis='y')
plt.title(ups)
...
plt.ylabel('$n_k$')
plt.xlabel('comoving {$k [1/R_1 H_1]$}')
