...
def nspcor(mfile, nm, spmask='Red'):
    if spmask == 'nomask':
        return gm(mfile,'nspK')
    elif spmask == 'Red':
        s0 = gm(mfile,'nspK_Red')
        m = gm(mfile,'mspM_Red')
        s1 = (gm(mfile,'L')**3)*np.dot(inv(m),s0/nm)
        return s1
    elif spmask == 'Vi':
        s0 = gm(f,'nspK_Vi')
        m = gm(f,'mspM_Vi')
...
class nspevol:
    def __init__(self, mfiles, spmask='Red'):
        self.sizeN = gm(mfiles[0],'sizeN')
        self.sizeL = gm(mfiles[0],'L')
        self.msa = gm(mfiles[0],'msa')
        self.nm = gm(mfiles[0],'nmodelist')
        self.avek = np.sqrt(gm(mfiles[0],'aveklist')/self.nm)*(2*math.pi/self.sizeL)
        # identify modes less than N/2
        self.k_below = np.sqrt(gm(mfiles[0],'aveklist')/self.nm) <= self.sizeN/2
        self.ttab = []
        self.logtab = []
        self.nsp = []
    ...
    def __init__(self, mfiles, spmask='Red'):
        ...
        for f in mfiles:
            if gm(f,'nsp?'):
                ...
                self.ttab.append(t)
                if spmask == 'nomask':
                    e0 = (self.avek**2)*gm(f,'nspK')/(t*(math.pi**2)*self.nm)
                    self.esp.append(e0)
                elif spmask == 'Red':
                    s0 = gm(f,'nspK_Red')
                    m = gm(f,'mspM_Red')
