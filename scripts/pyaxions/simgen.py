...
def _slurm_preamble(slurm, ranks, threads):
    """Return the login-shell SLURM preamble, or none when disabled."""
    if slurm is False or slurm is None:
        return []
    if slurm is True:
        slurm = {}
    if not isinstance(slurm, dict):
        raise TypeError('SLURM must be a bool or a dictionary of sbatch options')
    ...
    options = {
        ...
        'job-name': 'test_slurm',
        'nodes': 1,
        'ntasks-per-node': int(ranks),
        'cpus-per-task': int(threads),
        'mail-type': 'none',
        'time': '0:30:00',
    }
    options.update({str(key).replace('_', '-'): value
    ...
    for key, value in options.items():
        ...
        if not re.fullmatch(r'[A-Za-z0-9-]+', key):
            raise ValueError('invalid SBATCH option: %r' % key)
        if value is False or value is None:
            continue
        directive = '#SBATCH --' + key
        if value is not True:
            value = str(value)
            if '\n' in value or '\r' in value:
                raise ValueError('invalid newline in SBATCH option %s' % key)
            directive += '=' + shlex.quote(value)
        directives.append(directive)
    return directives


def _write_run_script(filename, ranks, threads, usa, grid, simu, phys, vqcp,
                      inco, out, extra='', slurm=False):
    ...
    use_slurm = slurm is not False and slurm is not None
    lines = [
        '#!/usr/bin/env bash',
        'set -o pipefail',
        '',
    ...
    if use_slurm:
        lines.extend([
        '    printf "Executing: srun"',
        '    printf " %q" "$@"',
        '    printf "\\n"',
        '',
        '    export OMP_NUM_THREADS="${SLURM_CPUS_PER_TASK:-$THREADS}"',
        '    export OMP_DYNAMIC',
...
def _write_slurm_batch(filename, run_script, slurm, ranks, threads,
                       modules=None, runs=None):
    """Write the allocation wrapper which may invoke the run script repeatedly."""
    if modules is None:
        modules = ('intel/2024.0', 'impi/2021.11')
    if runs is None:
        runs = ('run',)
    ...
    script_path = os.path.abspath(os.fspath(run_script))
    relative_script = os.path.relpath(script_path, os.path.dirname(filename))
    if os.sep not in relative_script:
        relative_script = './' + relative_script
    command = shlex.quote(relative_script)

    lines = [
        '#!/bin/bash -l',
        '# Standard output, error, working directory, and job resources:',
    ]
    lines.extend(_slurm_preamble(slurm, ranks, threads))
    lines.extend([
        '',
        '# Load compiler and MPI modules (the same ones used to build jaxions).',
        'module purge',
    ])
    lines.extend('module load ' + shlex.quote(module) for module in modules)
    lines.extend([
        '',
        'export OMP_NUM_THREADS="$SLURM_CPUS_PER_TASK"',
        'export OMP_PLACES=cores',
        '',
        '# Run the program. Add more calls here to run several stages in one job:',
    ])
    lines.extend(command + (' ' + run if run else '') for run in runs)
    lines.append('# %s continue INDEX' % command)
    lines.append('')
...
def simgen(N=256, zRANKS=1, prec='single', dev='cpu', fftplan=64, lowmem=False, prop='rkn4', spec=False, fspec=False, steps=1000000, wDz=1.0, sst0=10, lap=1,
    ...
    if SLURM is not False and SLURM is not None:
        ...
        print('SLURM batch script written to %s' % slurm_path)
