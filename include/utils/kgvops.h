	...
	void	stringcorre	(void *data, void *result, FieldPrecision fPrec, int nr=1) // parms is a pointer to m,r and grads, result written to result
	{
		switch (fPrec)
		{
			case FIELD_SINGLE :
				...
				for (size_t ix=0; ix < Ly; ix++) {
					...
					switch(mask){
						...
						case SPMASK_VIL:
								...
								break;
						case SPMASK_VIL2:
									m2sa[odx] = (da[0]*da[0]+da[1]*da[1])/Rscale*(vA);
								break;
						case SPMASK_SAXI:
									m2sa[odx]   =  da[2] ;
									m2sax[odx]  =  da[3] ;
