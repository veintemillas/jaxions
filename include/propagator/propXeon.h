				    ...
				    const size_t Vo, const size_t Vf, FieldPrecision precision, const unsigned int bSizeX, const unsigned int bSizeY, const unsigned int bSizeZ)
{
	...
	const size_t n0    = ppar.n0 ;      // number of scalars here 2
	const size_t VOL   = Sf*(Lz+2*NN);  // distance between scalars
	const size_t VOLV  = Sf*(Lz+2);     // distance between scalars in v
	const double s2beta = ppar.beta;    // sin^2 of mixing angle axion-ALP
...
#undef	step
	...
	#define	step 2
		...
		const float zQ1 = (float) (mA2*Rf)*((float) ((1.0-s2beta)/2.0));
		const float zQ2 = (float) (mA2*Rf)*((float) (s2beta ));
		const float zQ1C = (float) (mA2*R2*R2)*((float) ((1.0-s2beta)/2.0));
		const float zQ2C = (float) (mA2*R2*R2)*((float) (s2beta ));
		const float zQ1CC = (float) (mA2*R2)*((float) ((1.0-s2beta)/2.0));
		const float zQ2CC = (float) (mA2*R2)*((float) (s2beta ));
		...
		const float __attribute__((aligned(Align))) cjgAux[8]  = { 1.f,-1.f, 1.f,-1.f, 1.f,-1.f, 1.f,-1.f };
#else
		const size_t XC = (Lx<<1);
		const size_t YC = (Lx>>1);
		...
		const float __attribute__((aligned(Align))) cjgAux[4]  = { 1.f,-1.f, 1.f,-1.f };
#endif
		...
		const _MData_ zQ1Vec  = opCode(mul_ps,opCode(set1_ps, zQ1),cjg);
		const _MData_ zQ2Vec  = opCode(mul_ps,opCode(set1_ps, zQ2),cjg);
		const _MData_ zQ1CVec  = opCode(mul_ps,opCode(set1_ps, zQ1C),cjg);
		const _MData_ zQ2CVec  = opCode(mul_ps,opCode(set1_ps, zQ2C),cjg);
		const _MData_ zQ1CCVec  = opCode(mul_ps,opCode(set1_ps, zQ1CC),cjg);
		const _MData_ zQ2CCVec  = opCode(mul_ps,opCode(set1_ps, zQ2CC),cjg);
...
LogMsg(VERB_PARANOID,"[pX] z0 %d zF %d zM %d bY %d bSizeZ %d bSizeY %d ",z0, zF, zM, bY, bSizeZ, bSizeY);LogFlush();
LogMsg(VERB_PARANOID,"[pX] Lz %lu VOL %lu VOLV %lu [NN %lu] [n0 %lu]",Lz, VOL,VOLV,NN,n0);LogFlush();

		/* loop over points inside blocks
		zz,yy,xC are grid sites, idx is constrained by VOL
		we write with a float pointer that has twice (complex scalar)
		idxP0 is constrained by 2VOL
		idxV0 constrained by 2VOLV
		The seconc complex scalar is inmediately after the first,
		idxP0 in (3-4)VOL
		idxV0 in (3-4)VOL */
		for (uint zT = 0; zT < zM; zT++)
		 for (uint yT = 0; yT < bY; yT++)
		  #pragma omp parallel default(shared)
		  {
		    _MData_ tmp, mel, mPx, mPy, mMx, tmp2, lap, mel_[2],acc_[2],mod2[2];
		    #pragma omp for collapse(3) schedule(static)
		    for (uint zz = 0; zz < bSizeZ; zz++) {
		     for (uint yy = 0; yy < bSizeY; yy++) {
		      for (uint xC = 0; xC < XC; xC += step) {
			...
			for (int nf =0; nf<2;nf++) {
				...
				idx = idx_ + nf*VOL;
				idxP0 = (idx<<1);
// if ( (idxP0+step*2-1)/(2*VOL) != nf) LogError("overflow!")

				mel = opCode(load_ps, &m[idxP0]);
				lap = opCode(set1_ps, 0.f);
// LogMsg(VERB_PARANOID,"[pX] read ");LogFlush();
				for (size_t nv=1; nv < NN+1; nv++)
				{
					if (X[0] < nv*step)
						idxMx = ((idx + XC - nv*step) << 1);
	...
	#if	defined(__AVX__)// || defined(__AVX512F__)
				mod2[nf] = opCode(add_ps, opCode(permute_ps, mPy, 0b10110001), mPy);
	#else
				mod2[nf] = opCode(add_ps, opCode(shuffle_ps, mPy, mPy, 0b10110001), mPy);
	#endif
					...
					switch	(VQcd & V_PQ) {
						case V_PQ1:
							mMx = opCode(sub_ps, lap,
											opCode(mul_ps, mel,
												opCode(sub_ps,
													opCode(mul_ps, opCode(sub_ps, mod2[nf], opCode(set1_ps, R2)),
														opCode(set1_ps, LL)),
													opCode(set1_ps, A))));
						break;
						case V_PQ3:
							// a = p^2 - R^2
							tmp2 = opCode(sub_ps, mod2[nf], opCode(set1_ps, R2));
							// b = 2R^2(1-R/p)
							tmp  = opCode(mul_ps, opCode(set1_ps, 2*R2),
											opCode(sub_ps,opCode(set1_ps, 1.f),
												opCode(div_ps,opCode(set1_ps, Rf),
													opCode(sqrt_ps, mod2[nf]))));
							// tmp(cuadratic) if p > R and tmp2(quartic) if p < R
							tmp = opCode(kkk_ps,tmp,tmp2,mod2[nf],opCode(set1_ps, R2));
						...
						case V_PQ2:
							mMx = opCode(sub_ps, lap,
											opCode(mul_ps, mel,
												opCode(sub_ps,
													opCode(mul_ps,
														opCode(sub_ps, opCode(mul_ps, mod2[nf], mod2[nf]), opCode(set1_ps, R4)),
															opCode(mul_ps, mod2[nf], opCode(set1_ps, LaLa))),
												opCode(set1_ps, A))));
						break;
					}
				/* mMx = mMx + VQCD part */
			...
			if ( !(VQcd & V_EVOL_RHO) )
				switch	(VQcd & V_QCD) {
					...
					{
						// sin
						tmp2 = opCode(im_f1sq_f2_ps, mel_[0], mel_[1]);
						acc_[0] = opCode(add_ps, acc_[0],
												opCode(div_ps,
													opCode(mul_ps, opCode(cswap_ps, mel_[0]),
														opCode(mul_ps, zQ1CVec, tmp2)),
															opCode(mul_ps,
																opCode(mul_ps, mod2[0],mod2[0]),
																	opCode(sqrt_ps,mod2[1]))));
						acc_[1] = opCode(add_ps, acc_[1],
												opCode(div_ps,
													opCode(mul_ps, opCode(cswap_ps,mel_[1]),
														opCode(mul_ps, zQ2CVec, tmp2)),
															opCode(mul_ps,
																opCode(mul_ps, mod2[0],mod2[1]),
																	opCode(sqrt_ps,mod2[1]))));
						// sin
						// tmp2 = opCode(im_f1sq_f2_ps, mel_[0], mel_[1]);
						// acc_[0] = opCode(add_ps, acc_[0],
						// 						opCode(div_ps,
						// 							opCode(mul_ps, opCode(cswap_ps, mel_[0]),
						// 								opCode(mul_ps, zQ1CCVec, tmp2)),
						// 									opCode(mul_ps,
						// 										mod2[0],
						// 											opCode(sqrt_ps,mod2[0]))));
						// acc_[1] = opCode(add_ps, acc_[1],
						// 						opCode(div_ps,
						// 							opCode(mul_ps, opCode(cswap_ps,mel_[1]),
						// 								opCode(mul_ps, zQ2CCVec, tmp2)),
						// 									opCode(mul_ps,
						// 										mod2[1],
						// 											opCode(sqrt_ps,mod2[0]))));
					} break;
					case V_QCD0:
					break;
					case V_QCD1: // will have a shift
					default:
					{
						acc_[0] = opCode(add_ps, acc_[0],
												opCode(mul_ps, zQ1Vec, opCode(cmul_ps, mel_[0], mel_[1])));
						acc_[1] = opCode(add_ps, acc_[1],
												opCode(mul_ps, zQ2Vec, opCode(cmul_ps, mel_[0], mel_[0])));
					} break;
				}


// LogMsg(VERB_PARANOID,"[pX] aa idx %lu idx_ %lu idxP0 %lu zz %u",idx,idx_,idxP0,zC);LogFlush();
for (int nf = 0; nf<n0;nf++)
{
			idx = idx_ + nf*VOLV;
			idxV0 = (idx-NSf) << 1;
// if (idxV0+step > (axion->Size()+2*axion->Surf())*axion->DataSize ()/sizeof(float)) LogError("overflow!")
			idx = idx_ + nf*VOL;
			idxP0 = (idx<<1);
			...
			if (VQcd & V_EVOL_THETA)
			{
				...
				//5.- (-ar*mi ai*mr, -ar*mi ai*mr)*(-mi mr)/|m|^2 + R''/R (mr mi)
				mMx   = opCode(add_ps,
									opCode(div_ps, opCode(mul_ps, lap, vecma), mod2[nf]),
										opCode(mul_ps, mel, opCode(set1_ps, A)));
				//6.- (-vr*mi vi*mr, -vr*mi vi*mr)*(-mi mr)/|m|^2 + R/R (mr mi)
				mPy   = opCode(add_ps,
									opCode(div_ps, opCode(mul_ps, lap, vecmv), mod2[nf]),
...
tmp = opCode(sub_ps,
	...
	opCode(mul_ps, opCode(mul_ps, opCode(set1_ps, epsi), opCode(div_ps, mel, mod2[nf])),
		opCode(fmadd_ps, opCode(sub_ps,vecmv,opCode(mul_ps,mod2[nf],opCode(set1_ps, Rp))), opCode(set1_ps, 2.f), opCode(mul_ps, vecma, opCode(set1_ps, dzc)))));
#else
...
tmp = opCode(sub_ps,
	...
	opCode(mul_ps, opCode(mul_ps, opCode(set1_ps, epsi), opCode(div_ps, mel, mod2[nf])),
		opCode(add_ps,
			opCode(mul_ps, opCode(sub_ps,vecmv,opCode(mul_ps,mod2[nf],opCode(set1_ps, Rp))), opCode(set1_ps, 2.f)),
			...
			if (VQcd & V_EVOL_RHO)
			{
				...
				tmp   = opCode(div_ps, opCode(mul_ps, mel, vecma), mod2[nf]);
			}
...
#undef	step
	...
	LogMsg(VERB_PARANOID,"[pX] exit ");LogFlush();
