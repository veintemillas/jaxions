				...
				const size_t Vo, const size_t Vf, FieldPrecision precision, const unsigned int bSizeX, const unsigned int bSizeY, const unsigned int bSizeZ)
{
	...
	#define	step 1
		...
		const double ood2 = ppar.ood2a/(2.0*cmA);
		const double i4R2 = 1.0/(4.0*R*R);
		...
		/* mPx is intermediate value of P and is used to compute the next kick
			...
			// If potentials matter uncomment and expand the following */

			// switch	(VQcd & VQCD_TYPE) {
			// 	default:
			// 	case	VQCD_1:
				mPy = opCode(mul_pd, opCode(mul_pd, opCode(set1_pd, i4R2), tmp), mPx);
			// 	break;
			// }

			/* multiply by I = permutation + mpVec */

#if	defined(__AVX512F__)
			tmp = opCode(mul_pd, mpVec, opCode(castsi512_pd, opCode(shuffle_epi32, opCode(castpd_si512, mPy), _MM_PERM_BADC)));
#elif	defined(__AVX__)
			tmp = opCode(mul_pd, mpVec, opCode(permute_pd, mPy, 0b00000101));
#else
			tmp = opCode(mul_pd, mpVec, opCode(shuffle_pd, mPy, mPy, 0b00000001));
#endif

#if	defined(__AVX512F__) || defined(__FMA__)
			mPy = opCode(fmadd_pd, tmp, opCode(set1_pd, dzc), mPx);
#else
			mPy = opCode(add_pd, mPx, opCode(mul_pd, tmp, opCode(set1_pd, dzc)));
#endif
			/* final value of P in mPy */

			opCode(stream_pd, &m2[idxP0], mPy);
		      }
		    }
		  }
		}
...
#undef	step
	...
	#define	step 2
		...
		// inverse lattice spacing^2/2 cmA
		const float ood2 = ppar.ood2a/(2.0*cmA);
		const float i4R2 = 1.0/(4.0*R*R);
...
LogMsg(VERB_DEBUG,"[pX] z0 %d zF %d zM %d bY %d bSizeZ %d bSizeY %d [NN %d]",z0, zF, zM, bY, bSizeZ, bSizeY, NN);LogFlush();
		...
		/* Laplacian stored in lap including 1/2cmA */
			...
			// 	default:
			// 	case	VQCD_1:
				mPy = opCode(mul_ps, opCode(mul_ps, opCode(set1_ps, i4R2), tmp), mPx);
			// 	break;
			// }

#if	defined(__AVX__)// || defined(__AVX512F__)
			tmp = opCode(mul_ps, mpVec, opCode(permute_ps, mPy, 0b10110001));
#else
			tmp = opCode(mul_ps, mpVec, opCode(shuffle_ps, mPy, mPy, 0b10110001));
#endif

#if	defined(__AVX512F__) || defined(__FMA__)
			mPy = opCode(fmadd_ps, tmp, opCode(set1_ps, dzc), mPx);
#else
			mPy = opCode(add_ps, mPx, opCode(mul_ps, tmp, opCode(set1_ps, dzc)));
#endif
			opCode(stream_ps, &m2[idxP0], mPy);	// Avoids cache thrashing
		      }
		    }
		  }
		}
