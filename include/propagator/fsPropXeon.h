...
inline	void	fsAccKernelXeon(vvoid * __restrict__ v_, void * __restrict__ m2_, double *R, const double dz, const double c, const double d,
				const double ood2, const double LL, const double aMass2, const double gamma, const double fMom, const size_t Lx, const size_t Vo, const size_t Vf, FieldPrecision precision)
{
	...
	#define	step 1
		...
		double       * __restrict__ m2	= (const double * __restrict__) __builtin_assume_aligned (m2_, Align);
...
#undef	step
	...
	#define	step 2
		...
		float     * __restrict__ m2	= (const float * __restrict__) __builtin_assume_aligned (m2_, Align);
				...
				const double ood2, const double LL, const double aMass2, const double gamma, const double fMom, const size_t Lx, const size_t Vo, const size_t Vf, FieldPrecision precision)
{
	...
	#define	step 2
		...
		const unsigned int __attribute__((aligned(Align))) iIddxx[8] = { 0, 1, 2, 3, 4, 5, 6, 7};

	#elif	defined(__AVX__)
		const unsigned int __attribute__((aligned(Align))) iIddxx[4]  = { 0, 1, 2, 3};
	#else
		const unsigned int __attribute__((aligned(Align))) iIddxx[2]  = { 0, 1 };
	#endif
		...
		#pragma omp parallel default(shared)
		{
			...
			for (size_t oy = 0; oy < Ly; oy++)	// As Javier pointed out, the transposition makes y the slowest coordinate
			{
				...
				for (size_t oz = 0; oz < Tz; oz++)
				{
					...
					for (size_t ox = 0; kx < Lx; kx += step)
					{
						...
						pxV = opCode(add_epi32,opCode(load_epi32,kx),iIddxx);
						//calculate p2
						px^2+pypz2
						 
						aux = opCode(mul_epi32,idxV, idxV);

						// v = v + m2 dct
						// v = v*p
						// v = v Cos[pdt] - m Sin[pdt]
						// m = v Sin[pdt] - m Cos[pdt]
						// v = v/p
					}
		}
		}
			...
			for (size_t idx = Vo; idx < Vf; idx += step)
			{
				...
				// moreover, if N is a divisor of 16, if idx is in (0,N-1) also idx+15 will
				// therefore we just need to naively add from idx
				tmp = idx/Lx;
				...
				// calculation of n in (-n/2+1,n/2) from (0,N-1+step)
