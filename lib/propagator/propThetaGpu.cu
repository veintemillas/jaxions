...
template<typename Float>
static __device__ __forceinline__ void	propagateThetaCoreGpu(const uint idx, const Float * __restrict__ m, Float * __restrict__ v, Float * __restrict__ m2, const Float zP, const Float tPz,
...
template<typename Float>
__global__ void	propagateThetaKernel(const Float * __restrict__ m, Float * __restrict__ v, Float * __restrict__ m2, const Float zP, const Float tPz, const Float iz, const Float zQ,
		     ...
		     const double nQcd, const uint Lx, const uint Lz, const uint Vo, const uint Vf, FieldPrecision precision, cudaStream_t &stream)
{
	...
	if (precision == FIELD_DOUBLE)
	{
		...
		propagateThetaKernel<<<gridSize,blockSize,0,stream>>>((const double *) m, (double *) v, (double *) m2, M_1_PI*iz, tPz, iz, zQ, dzc, dzd, ood2, Lx, Lx*Lx, Vo, Vf);
	}
	else if (precision == FIELD_SINGLE)
	{
		...
		propagateThetaKernel<<<gridSize,blockSize,0,stream>>>((const float *) m, (float *) v, (float *) m2, (float) (M_1_PI*iz), tPz, iz, zQ, dzc, dzd, ood2, Lx, Lx*Lx, Vo, Vf);
	}
}
