...
#endif
void	spropThetaKernelXeon(const void * __restrict__ m_, void * __restrict__ v_, void * __restrict__ m2_, double *z, const double dz, const double c, const double d,
	...
	#define	step 4
		...
		#pragma omp parallel default(shared)
		{
			...
			for (size_t idx = Vo; idx < Vf; idx += step)
			{
				...
				//lap will miss some factor like (2pi/L)^21/N^3 that will be stored in d2Vec
				lap = opCode(load_ps, &m2[idx]);
				...
				acc = opCode(sub_ps,
					opCode(mul_ps, lap, d2Vec), opCode(mul_ps, zQVec, sel));

				vel = opCode(load_ps, &v[idxMz]);

#if	defined(__MIC__) || defined(__AVX512F__) || defined(__FMA__)
				vel = opCode(fmadd_ps, acc, dzcVec, vel);
				mel = opCode(fmadd_ps, tmp, dzdVec, mel);
#else
				vel = opCode(add_ps, vel, opCode(mul_ps, acc, dzcVec));
				mel = opCode(add_ps, mel, opCode(mul_ps, vel, dzdVec));
#endif
				/*	Make sure the result is between -pi and pi	*/
				...
				opCode(store_ps, &v[idx - Sf], vel);
				opCode(store_ps, &m[idx],  mel);
			}
		}
#undef	_MData_
#undef	step
...
// void	spropThetaXeon	(Scalar *axionField, const double dz, const double delta2, const double nQcd, const size_t Lx, const size_t V, const size_t S, FieldPrecision precision)
// {
// #ifdef USE_XEON
...
// 	{
// 		spropThetaKernelXeon(mX, vX, m2X, z, dz, C1, D1, ood2, nQcd, Lx, 2*S, V, precision);
// 	}
// 	axionField->exchangeGhosts(FIELD_M);
// 	#pragma offload target(mic:micIdx) in(z:length(8) UseX) nocopy(mX, vX, m2X : ReUseX)
// 	{
// 		spropThetaKernelXeon(mX, vX, m2X, z, dz, C1, D1, ood2, nQcd, Lx, S, 2*S, precision);
// 		spropThetaKernelXeon(mX, vX, m2X, z, dz, C1, D1, ood2, nQcd, Lx, V, ext, precision);
// 	}
...
// 	#pragma offload target(mic:micIdx) in(z:length(8) UseX) nocopy(mX, vX, m2X : ReUseX) signal(&bulk)
// 	{
// 		spropThetaKernelXeon(m2X, vX, mX, z, dz, C2, D2, ood2, nQcd, Lx, 2*S, V, precision);
// 	}
// 	axionField->exchangeGhosts(FIELD_M2);
// 	#pragma offload target(mic:micIdx) in(z:length(8) UseX) nocopy(mX, vX, m2X : ReUseX)
// 	{
// 		spropThetaKernelXeon(m2X, vX, mX, z, dz, C2, D2, ood2, nQcd, Lx, S, 2*S, precision);
// 		spropThetaKernelXeon(m2X, vX, mX, z, dz, C2, D2, ood2, nQcd, Lx, V, ext, precision);
// 	}
// 	#pragma offload_wait target(mic:micIdx) wait(&bulk)
//
// 	*z += dz*D2;
...
// 	#pragma offload target(mic:micIdx) in(z:length(8) UseX) nocopy(mX, vX, m2X : ReUseX) signal(&bulk)
// 	{
// 		spropThetaKernelXeon(mX, vX, m2X, z, dz, C3, D3, ood2, nQcd, Lx, 2*S, V, precision);
// 	}
// 	axionField->exchangeGhosts(FIELD_M);
// 	#pragma offload target(mic:micIdx) in(z:length(8) UseX) nocopy(mX, vX, m2X : ReUseX)
// 	{
// 		spropThetaKernelXeon(mX, vX, m2X, z, dz, C3, D3, ood2, nQcd, Lx, S, 2*S, precision);
// 		spropThetaKernelXeon(mX, vX, m2X, z, dz, C3, D3, ood2, nQcd, Lx, V, ext, precision);
// 	}
// 	#pragma offload_wait target(mic:micIdx) wait(&bulk)
//
// 	*z += dz*D3;
...
// 	#pragma offload target(mic:micIdx) in(z:length(8) UseX) nocopy(mX, vX, m2X : ReUseX) signal(&bulk)
// 	{
// 		spropThetaKernelXeon(m2X, vX, mX, z, dz, C4, D4, ood2, nQcd, Lx, 2*S, V, precision);
// 	}
// 	axionField->exchangeGhosts(FIELD_M2);
// 	#pragma offload target(mic:micIdx) in(z:length(8) UseX) nocopy(mX, vX, m2X : ReUseX)
// 	{
// 		spropThetaKernelXeon(m2X, vX, mX, z, dz, C4, D4, ood2, nQcd, Lx, S, 2*S, precision);
// 		spropThetaKernelXeon(m2X, vX, mX, z, dz, C4, D4, ood2, nQcd, Lx, V, ext, precision);
// 	}
// 	#pragma offload_wait target(mic:micIdx) wait(&bulk)
//
// 	*z += dz*D4;
