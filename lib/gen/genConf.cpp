...
void	ConfGenerator::confthermal(Cosmos *myCosmos, Scalar *axionField)
{
	...
	this way  w/T = sqrt( (2pi n /LR)^2 + m^2)/T
	is now    w/T = sqrt( n^2 + m^2R^2/k0^2) / TR/k0 )

	TR / k0 = TR  / (2pi/L) = pTf N / 2
	*/
	...
	double R    = *axionField->RV();
	double mS2  = 2*axionField->LambdaP()*( (RPQ > R) ? 0.5*(RPQ*RPQ-R*R) : R*R-RPQ*RPQ);
	double L   = axionField->BckGnd()->PhysSize();
	double N   = axionField->Length();
	double k0   = 2.0*M_PI/L;
	mS2 /= k0*k0; //mS is normalised to k0
	...
	/* Normalisation
	See the notes or the jupyter example for the justification */
	double norma1 = 3 * RPQ*RPQ / (pow(M_PI * ic.kcr * N,3));

	LogMsg(VERB_NORMAL,"L %e, norma1 %e \n",L, norma1);
	scaleField (axionField, FIELD_M, sqrt(norma1));

	double norma2 = 3 * RPQ*RPQ / (M_PI * ic.kcr * N * L * L);
...
void	ConfGenerator::confstring2(Cosmos *myCosmos, Scalar *axionField)
{
	...
	if (myCosmos->ICData().normcore){
		...
		// Set velocity to zero after relaxing!
