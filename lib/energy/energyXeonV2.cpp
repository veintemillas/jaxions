	...
	#define	step 1
		...
		#pragma omp parallel default(shared) 
		{
			...
			for (size_t idx = Vo; idx < Vf; idx += step)
			{
				...
				mTp = opCode(sub_pd, mod, opCode(set1_pd, 1.0));
				mod = opCode(mul_pd, mTp, mTp);
				mTp = opCode(sub_pd, opCode(set1_pd, 1.0), opCode(mul_pd, mel, ivZ));

				/****************
 				 * POTENTIAL V2 *
 				 ****************/

				mel = opCode(mul_pd, mTp, mTp);
				
#ifdef	__MIC__
				mTp = opCode(mul_pd, opCode(add_pd, opCode(castsi512_pd, opCode(shuffle_epi32, opCode(castpd_si512, mel), _MM_PERM_BADC)), mel), opCode(set1_pd, 0.5));
//				vel = opCode(castsi512_pd, opCode(shuffle_epi32, opCode(castpd_si512, mTp), _MM_PERM_BADC));
				tVp = opCode(mask_blend_pd, opCode(int2mask, 0b0000000010101010), mod, mTp);
#elif defined(__AVX__)
				mTp = opCode(mul_pd, opCode(add_pd, opCode(permute_pd, mel, 0b00000101), mel), opCode(set1_pd, 0.5));
				tVp = opCode(blend_pd, mod, mTp, 0b00001010);
#else
				mTp = opCode(mul_pd, opCode(add_pd, opCode(shuffle_pd, mel, mel, 0b00000001), mel), opCode(set1_pd, 0.5));
		...
		const double o2 = ood2*0.25;//0.375;
...
void	energyCpuV2	(Scalar *axionField, const double delta2, const double LL, const double nQcd, const size_t Lx, const size_t V, const size_t S, FieldPrecision precision, void *eRes)
{
	...
	axionField->exchangeGhosts(FIELD_M);
	energyKernelXeonV2(axionField->mCpu(), axionField->vCpu(), z, ood2, LL, nQcd, Lx, S, V+S, precision, eRes, 0.0);
}
