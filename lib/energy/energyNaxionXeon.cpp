...
void	energyNaxionKernelXeon(const void * __restrict__ m_, const void * __restrict__ v_, void * __restrict__ m2_, double *R, double *z, double frw, const double ood2, const double aMass2,
...
void	energyNaxionCpu	(Scalar *axionField, const double delta2, const double aMass2, void *eRes, const bool map)
{
	...
	switch	(map) {
		case	true:
			energyNaxionKernelXeon<true>(axionField->mCpu(), axionField->vCpu(), axionField->m2Cpu(), R, z, frw, ood2, aMass2, Lx, Vo, Vf, precision, eRes);
			break;

		case	false:
			energyNaxionKernelXeon<false>(axionField->mCpu(), axionField->vCpu(), axionField->m2Cpu(), R, z, frw, ood2, aMass2, Lx, Vo, Vf, precision, eRes);
			break;
	}
}
