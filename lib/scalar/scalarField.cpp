...
// ----------------------------------------------------------------------
void	Scalar::writeMAPTHETA (double zzz, const int index, void *contbin, int numbins)
{
...
double	Scalar::thetaDIST(int numbins, void * thetabin)//int *window)
{
	...
	if(fieldType == FIELD_SAXION) {
		if (precision == FIELD_DOUBLE)
			thetamaxi = find<FIND_MAX, complex<double>>(static_cast<complex<double>*>(m) + n2, n3, [] (complex<double> x) { return abs(arg(x)); });
		else
			thetamaxi = find<FIND_MAX, complex<float>> (static_cast<complex<float> *>(m) + n2, n3, [] (complex<float>  x) { return ((double) abs(arg(x))); });
	} else {
		if (precision == FIELD_DOUBLE)
			thetamaxi = find<FIND_MAX, double>(static_cast<double*>(m) + n2, n3, [] (double x) { return abs(x); });
		else
			thetamaxi = find<FIND_MAX, float> (static_cast<float *>(m) + n2, n3, [] (float  x) { return ((double) abs(x)); });
	}
